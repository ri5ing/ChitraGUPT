...
import { ChevronDown, ChevronUp, CheckSquare, Clock } from "lucide-react";
...
export default function ReviewQueuePage() {
  ...
  const reviewQueueQuery = useMemoFirebase(() => {
    ...
    return query(
      ...
      where('status', 'in', ['accepted', 'pending_approval']) // 'accepted' is now 'In Review' for the auditor, 'pending_approval' is waiting for client
    );
  }, [firestore, user]);
          ...
          <TableRow>
            ...
            <TableCell>
              <Badge variant={getStatusVariant(request.status)} className="capitalize">
                 {request.status === 'pending_approval' && <Clock className="mr-1 h-3 w-3" />}
            ...
            <TableCell className="text-right space-x-2">
                ...
                {auditorProfile && request.status === 'accepted' && (
